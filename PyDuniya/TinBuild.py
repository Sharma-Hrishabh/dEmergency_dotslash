...
def draw_triangles(builder, sites):
    ...
    draw_triangle(base)
    return [float(x_plot)+10.0,float(y_plot)-10.0]
...
if __name__=="__main__":
    ...
    if not doHelp:
        ...
        x_plot = input("enter the x coordinate ")
        y_plot = input("enter the y coordinate ")
        f = open(args[0], "a")
        f.write(x_plot+" "+y_plot+'\n')
        f.close()
        if len(args) > 0:
            fp = open(args[0],'r')
        for line in fp:
    ...
    if c.plot:
        ...
        plt.plot([p.x for p in pts], [p.y for p in pts],'go')
        plt.plot(float(x_plot), float(y_plot),'ro')
        ...
        for i in range(0, len(pts)):
            print((pts[i].sitenum, pts[i].x, pts[i].y))
            #print "p %d %.3f %.3f" %(pts[i].sitenum, pts[i].x, pts[i].y)
        for i in range(0, len(lts)):
            print((lts[i].sitenum, lts[i].x, lts[i].y))
